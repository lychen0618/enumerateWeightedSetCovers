...
public class EnumerateSetCovers {
     ...
     */
    public void enumerate(SetCoverProblem problem, int maxResults, boolean nonRedundant) throws IOException {
        ...
        while (!pq.isEmpty()) {
            ...
            if (foundNew) {
                ...
                // 存入printAlready，nonRedundant为假时要注释掉
                // printedAlready.add((BitSet) newSolution.getSolution().clone());
        ...
        OutputStreamWriter osw=new OutputStreamWriter(new FileOutputStream("D://output//esc.txt"));
        int index=1;
        for (BitSet bs : printedAlready) {
            osw.write(String.format("%d: ", index));
